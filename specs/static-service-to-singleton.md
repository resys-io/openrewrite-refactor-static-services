...
| updateFields                     | When set to `true`, remove static modifier from non-public fields are accessed by methods whose static modifier is removed. Unless field is final and type is known to be immutable (primitive, String, enum etc.). Or name is written in all capital letters (then we'll assume it to be a constant). |
| updateWrittenFields              | When set to `true`, remove static modifier from public fields, that are written by methods whose static modifier is removed. |
| updateAccessedFields             | When set to `true`, remove static modifier from public fields, that are accessed by methods whose static modifier is removed. Unless field is final and type is known to be immutable (primitive, String, enum etc.). Or name is written in all capital letters (then we'll assume it to be a constant). |
